...
### D-12-13 영상 촬영 방향 — Landscape 전용 (2026-05-12 추가)
- **결정**: 모든 영상은 **가로(landscape) 방향으로만 촬영**. 세로(portrait) 촬영 미지원.
  - 해상도: **1280×720 (16:9 landscape)** 고정 — D-12-03 보강
  - 캡처 화면 UI는 가로 모드로 강제 (디바이스 회전 잠금 / 회전 안내)
  - 디바이스가 세로 상태로 진입 시: "영상은 가로로 촬영해 주세요" 안내 + 회전 가이드 일러스트
  - 실제 녹화는 디바이스가 landscape 방향으로 회전된 상태에서만 가능
- **표시 측 영향**:
  - `group-detail` 영상 카드: **16:9 landscape 비율**로 표시 (정사각형/9:16 폐기)
  - `chat` 영상 인용 카드: 16:9 썸네일
- **메타 검증**:
  - Storage 업로드 시 영상 width/height 메타 확인
  - landscape 비율(width > height) 아닌 경우 업로드 거부 (서버 측 Cloud Function 검증)
- **근거**:
  - 매시 3초 영상을 그룹 내 여러 멤버가 함께 보는 UX 컨셉 — landscape가 풍경/단체샷에 적합
  - 디바이스별 portrait 영상의 letterbox 회피 (디자인 일관성)
  - 영상 코덱/해상도 통일로 재생 안정성 향상
